...
export class Controller {
  ...
  render () {
    ...
    const timeLabel = ctrl.append('div').style('margin-top', '10px')
    const progresser = ctrl
      .append('div')
    ...
    timer(() => {
      ...
      if (this.stage.playing) {
        const node = progresser.node()
        if (node) {
          node.value = this.stage.sec.toString()
        }
      }
  ...
  makeDraggable (contentID: string, draggerID: string) {
    ...
    if ((div != null) && (dragger != null)) dragElement(div, dragger)
